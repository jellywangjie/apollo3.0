...
function main(){
    ...
    docker ps -a --format "{{.Names}}" | grep 'apollo_dev_jwang' 1>/dev/null
    if [ $? == 0 ]; then
        docker stop apollo_dev_jwang 1>/dev/null
        docker rm -v -f apollo_dev_jwang 1>/dev/null
    fi
    ...
    info "Starting docker container \"apollo_dev_jwang\" ..."
    docker run -it \
        -d \
        --privileged \
        --name apollo_dev_jwang \
        ${MAP_VOLUME_CONF} \
    ...
    if [ $? -ne 0 ];then
        error "Failed to start docker container \"apollo_dev_jwang\" based on image: $IMG"
        exit 1
    fi

    if [ "${USER}" != "root" ]; then
        docker exec apollo_dev_jwang bash -c '/apollo/scripts/docker_adduser.sh'
    fi
